...
class TestPaperBuyRiskGate:
    ...
    def test_paper_buy_blocked_by_notional_cap(self, paper_engine, monkeypatch):
        ...
        assert result.executed is False
        assert result.blocked is True
        assert "above_max_trade_notional" in (result.reason or "")
...
class TestRejectionPersistence:
    def test_rejection_emits_event_log(self, paper_engine, db_session, monkeypatch):
        ...
        assert "above_max_trade_notional" in warns[-1].message
